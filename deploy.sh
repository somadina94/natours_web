#!/bin/bash
# =============================================================================
# deploy.sh — Production VPS deployment for Natours Web
# =============================================================================
#
...
echo "Starting VPS deployment of Natours Web..."
...
read_env_var NATOURS_WEB_IMAGE
read_env_var NEXT_PUBLIC_API_BASE_URL
read_env_var NEXT_PUBLIC_SITE_URL
read_env_var NEXT_PUBLIC_MEDIA_BASE_URL

COMPOSE_FILE="docker-compose.vps.yml"
PROJECT="natours-web-vps"
DEPLOY_LOCK="${TMPDIR:-/tmp}/natours-web-deploy.lock"
DEFAULT_IMAGE="ghcr.io/somadina94/natours_web:latest"
NATOURS_WEB_IMAGE="${NATOURS_WEB_IMAGE:-$DEFAULT_IMAGE}"
DEPLOY_BUILD_LOCAL="${DEPLOY_BUILD_LOCAL:-0}"
export NATOURS_WEB_IMAGE
...
stop_own_stack() {
    echo "Stopping Natours Web stack (if running)..."
...
pull_image() {
    echo "Pulling prebuilt image: ${NATOURS_WEB_IMAGE}"
    login_ghcr
    local attempt
    for attempt in 1 2 3; do
        if docker pull "${NATOURS_WEB_IMAGE}"; then
            return 0
        fi
        echo "Pull failed (attempt ${attempt}/3), retrying in 15s..."
        sleep 15
    done
    echo "Failed to pull ${NATOURS_WEB_IMAGE}"
...
build_image_local() {
    ...
    NATOURS_WEB_IMAGE="${PROJECT}-web:latest"
    export NATOURS_WEB_IMAGE
}
...
echo "API: ${NEXT_PUBLIC_API_BASE_URL:-https://natoursapi.jahbyte.com/api/v1/}"
